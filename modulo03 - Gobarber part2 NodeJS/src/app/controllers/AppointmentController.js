...
class AppointmentController {
  ...
  async store(req, res) {
    ...
    /**
     * Check if userId is diferent of provider_id
     */
    if (req.userId === provider_id) {
      return res
        .status(401)
        .json({ error: 'You can`t create appointments to your self!' });
    }
    ...
    if (!checkIsProvider) {
      return res
        .status(401)
        .json({ error: 'You can olny create appointments whith providers!' });
    }

    const hourStart = startOfHour(parseISO(date));
    /**
     * Verifica se a data selecionada já passou da data de hoje
     * */
